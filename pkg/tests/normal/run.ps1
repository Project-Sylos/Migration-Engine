...
if (Test-Path "pkg/tests/normal/migration_test.db") {
    Write-Host "Removing pkg/tests/normal/migration_test.db file..." -ForegroundColor Yellow
    Remove-Item -Path "pkg/tests/normal/migration_test.db" -Force -ErrorAction SilentlyContinue
}

# # Remove the migration config YAML file if it exists
Remove-Item -Path "pkg/tests/normal/migration_test.yaml" -ErrorAction SilentlyContinue

# # Remove the spectra.db file if it exists
Remove-Item -Path "pkg/tests/normal/spectra.db" -ErrorAction SilentlyContinue
